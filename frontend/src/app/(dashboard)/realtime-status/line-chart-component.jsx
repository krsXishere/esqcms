...
const LineChartComponent = ({
  ...
  // compute last values per metricKey and derive a display value+unit
  const lastValueInfo = useMemo(() => {
    const lastForKey = new Map();
    // helper to get last non-null numeric for a key
    const getLast = (key) => {
    ...
    // decide which base to display (priority: current, temperature, vibration, else first)
    const priority = ["current", "temperature", "temp", "vibration", "vib"];
    let chosenBase = null;
  ...
  let label = null;
    ...
    if (chosenBase) {
      ...
      // pick item with largest magnitude (use absolute) among last values
      let best = null;
      ...
      if (best) {
        display = best.last;
        label = best.key; // keep the series key (e.g., 'X' or 'Vibration X')
      }

        // unit mapping based on chosen base
        if (chosenBase.includes("current")) unit = "A";
        else if (chosenBase.includes("temperature") || chosenBase.includes("temp")) unit = "°C";
        else if (chosenBase.includes("vibration") || chosenBase.includes("vib")) unit = "g";
        else unit = "";

        // If unit still unknown, try to infer from the chart title prop (e.g. "Vibration")
        if (!unit && typeof title === "string") {
          const t = title.toLowerCase();
          if (t.includes("current") || t.includes("amp") || t.includes("ampere")) unit = "A";
          else if (t.includes("temperature") || t.includes("temp") || t.includes("°c") ) unit = "°C";
          else if (t.includes("vibration") || t.includes("vib") || t.includes("accel")) unit = "g";
        }
    }

    return { value: display, unit, label };
  }, [data, metricKeys]);
          ...
          {lastValueInfo.value != null && (
            <Typography sx={{ fontWeight: 700 }}>
              {Number.isFinite(lastValueInfo.value) ? (
                lastValueInfo.label
                  ? `${String(lastValueInfo.label)}: ${Number(lastValueInfo.value).toFixed(2)} ${lastValueInfo.unit}`.trim()
                  : `${Number(lastValueInfo.value).toFixed(2)} ${lastValueInfo.unit}`.trim()
              ) : (
                "—"
              )}
            </Typography>
