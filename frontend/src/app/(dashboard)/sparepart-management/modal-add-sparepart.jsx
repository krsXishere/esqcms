...
const ModalAddSparepart = ({ open, setOpen, onSuccessAdd }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (result?.success) {
        enqueueSnackbar("Successfully added sparepart", { variant: "success" });
        onSuccessAdd();
        handleClose();
      } else {
        enqueueSnackbar("Failed to add sparepart", { variant: "error" });
      }
    } catch (error) {
      enqueueSnackbar(error.message || "Failed to add sparepart", { variant: "error" });
    }
  };
  ...
  useEffect(() => {
    const fetchAreas = async () => {
      try {
        ...
        if (result?.data) {
          const formatted = result.data.map((area) => ({
            label: area.name,
            value: area.id,
          }));
...
export default ModalAddSparepart;
