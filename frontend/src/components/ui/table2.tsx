...
export default function Table2({ data }: { data: Row2[] }) {
  ...
  const statusColor = (status: Row2["status"]) => {
    switch (status) {
      case "Approved":
        return "bg-gray-400";
      case "Revision":
            ...
            {paginatedData.map((row) => (
              <tr key={row.id} className="border-t border-gray-200">
                ...
                <td className="px-4 py-3">
                  <span className="inline-flex items-center justify-center rounded-full bg-gray-200 px-3 py-1 text-xs font-medium text-gray-700">
                    {row.type}
                  </span>
                </td>
